...
namespace Task2.Products.ProductPriceChanges
{
    public static class PriceReader
    {
        public static Dictionary<MeatCategory, decimal> GetMeatPriceChanges()
        {
            ...
            foreach (var line in File.ReadLines(@"D:\C# projects\SigmaHomework\Task2\Problem1\Products\ProductPriceChanges\MeatPriceChanges.txt"))
            {
        ...
        public static Dictionary<int, decimal> GetDairyProductsPriceChanges()
        {
            ...
            foreach (var line in File.ReadLines(@"D:\C# projects\SigmaHomework\Task2\Problem1\Products\ProductPriceChanges\DairyProductsPriceChanges.txt"))
            {
                string[] parts = line.Split(' ');
                if (parts.Length == 2)
                {
